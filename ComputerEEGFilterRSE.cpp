...
ComputerEEGFilterRSE::ComputerEEGFilterRSE() {
    channelParametersHat = generateChannelParameters(N_CHANNELS);
    pos = zeros<mat>(3, 1);
    ...
    b = join_cols(zeros<mat>(N_CHANNELS * F_channel.n_rows, 1), b_arm);

    timeBin = TIME_BIN;
    timeStep = -1;
}

map ComputerEEGFilterRSE::filter(mat obs) {
    timeStep++;
    ...
    //       think.
    x = join_cols(join_cols( \
        channelParametersHat.reshape(3 * N_LAGS * N_CHANNELS, 1, 1), pos), \
        uHistory);

    // Select the correct matrices.
    F_current = F.slice(timeStep);
    Q_current = Q.slice(timeStep);
    b_current = b.slice(timeStep);

    // PERFORM THE PREDICTION STEP.
    pred_x = F_current * x + b_current;
    pred_cov = F_current * covariance * F_current.t() + Q_current;
        
    // PERFORM THE UPDATE STEP.
    predicted_u = pred_x.submat(3 * N_LAGS * N_CHANNELS + 3, 0, \
        pred_x.n_rows - 1, 0);
    estimated_obs = channelParametersHat * predicted_u;

    // The derivative of the observation vector with respect to the state,
    // evaluated at estimated_obs.
    D_obs = zeros<mat>(pred_x.n_rows, N_CHANNELS);
    for(int c=0; c<N_CHANNELS; c++)
    {
    ...
    DD_obs = zeros<cube>(pred_x.n_rows, pred_x.n_cols, N_CHANNELS);
    for(int c=0; c<N_CHANNELS; c++)
    {
    ...
    cov_adjust = pred_cov.zeros();
    for(int c = 0; c < N_CHANNELS; c++)
    {
        cov_adjust += 1 / channelVariances(c) * \
            (trans(D_obs.col(c)) * D_obs.col(c) + \
            (estimated_obs(c, 0) - obs(c, 0)) * DD_obs.slice(c));
    }
    ...
    // value.
    x_adjust = pred_x.zeros();
    for(int c = 0; c < N_CHANNELS; c++)
    {
        x_adjust += 1 / channelVariances(c) * \
            (obs(c, 0) - estimated_obs(c, 0)) * D_obs.col(c);
    }
    new_x = pred_x + new_cov * x_adjust;

    // UPDATE THE CLASS VARIABLES
    new_channelParametersHat = new_x.submat(0, 3 * N_LAGS * N_CHANNELS - 1);
    channelParametersHat = \
        trans(new_channelParametersHat.reshape(3 * N_LAGS, N_CHANNELS));
