...
class TrackStatus(gtk.DrawingArea):
	...
	validity_r, age_r, camera_pos_r, screen_pos_r = self.find_gaze('right')
	validity_l, age_l, camera_pos_l, screen_pos_l = self.find_gaze('left')
	gaze_x = -1000
	gaze_y = -1000
	if (validity_r <= 1) and (validity_l <= 1) :
		gaze_x = 1920 * (screen_pos_r.x + screen_pos_l.x) / 2.0
		gaze_y = 1080 * (screen_pos_r.y + screen_pos_l.y) / 2.0

	print gaze_x
	print gaze_y

	socket.send("%d %d" % (gaze_x, gaze_y))
