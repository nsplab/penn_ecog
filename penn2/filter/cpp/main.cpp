...
int main(int argc, char** argv) {
    ...
    INIReader reader("../../../config/filter.cfg");
    if (reader.ParseError() < 0) {
        std::cout << "Can't load '../../../config/filter.cfg'\n";
        return 1;
    }
    ...
    int useRSE = reader.GetInteger("filter", "useRSE", 1);                            // boolean variable determines whether the state equation used will be the RSE (relevant to jointrse_filter.cpp)
    int timeInvariant = reader.GetInteger("filter", "timeInvariant", 0);              // boolean variable determines whether the state equation used will be a time invariant equation (relevant to jointrse_filter.cpp)

                                                                // parameters specific to jointRSE
    double trialTime = reader.GetReal("filter", "trialTime", 3.0);                 // (in seconds) determines arrival time assumed by the reach state equation. (relevant to jointrse_filter.cpp)
    double diagQ = reader.GetReal("filter", "diagQ", 1.0e-3);                      // increment covariance (duplicated on the diagonal (?)) of the random walk on which the RSE is based
    double finalPosCov = reader.GetReal("filter", "finalPosCov", 1.0e-6);          // covariance of the final position (used by the RSE; relevant to jointrse_filter.cpp)
    double finalVelCov = reader.GetReal("filter", "finalVelCov", 1.0e-8);          // covariance of the final velocity (used by the RSE; relevant to jointrse_filter.cpp)
    double channelCov = reader.GetReal("filter", "channelCov", 1.0);               // ?? not sure... related to noise in the observation model?
    double initialArmPosVar = reader.GetReal("filter", "initialArmPosVar", 1.0);   // covariance of the initial position (used by the RSE; relevant to jointrse_filter.cpp)
    double initialArmVelVar = reader.GetReal("filter", "initialArmVelVar", 1.0);   // covariance of the initial velocity (used by the RSE; relevant to jointrse_filter.cpp)
    ...
    INIReader readerFeature(featureConfig.c_str());

    if (reader.ParseError() < 0) {
        std::cout << "Can't load 'featureConfig'\n";
        return 1;
    }

    unsigned featureRate = readerFeature.GetInteger("feature", "outputRate", 10);      // retrieve the rate at which the features (typically, power in various frequency bands) are being updated.
