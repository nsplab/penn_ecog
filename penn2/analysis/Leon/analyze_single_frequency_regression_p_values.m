function analyze_single_frequency_regression_p_values(power_matrix, large_labels, F, channel_id, fourier_sampling_rate, limit_freq, p_value_limit,normalized_flag, log_flag)
%ANALYZE_SINGLE_FREQUENCY_REGRESSION_P_VALUES generates one panel that shows
%the p values for individual regressions in each of
...
xlabel('Frequency [Hz]')
ylabel('P-values')
title(['P-Values for all the trials ( ' num2str(trials) ' )']);
%text(1000,0.025,);
ylimit=get(gca,'YLim');
xlimit=get(gca,'XLim');
...
if nan_count == 0, %If there is not a single nan
    hl = legend([p2, p3],{'P-values', ['P >=' num2str(p_value_limit)]},'Location','Best');
else
    hl = legend([p1, p2, p3],{'NaN' ,'P-values', 'P >=0.05'},'Location','Best');
end
newPosition = [0.8 0.85 0.2 0.2];
newUnits = 'normalized';
